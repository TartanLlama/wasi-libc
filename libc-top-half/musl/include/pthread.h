...
#define pthread_create(...)                                                    \
  ({                                                                           \
    _Static_assert(                                                            \
        0, "This function is not available on a single-threaded target; "      \
           "switch to a multi-threaded target with -pthread or enable a stub " \
           "implementation by undefining _WASI_STRICT_PTHREAD");               \
    0;                                                                         \
  })
