...
#if defined __wasilibc_unmodified_upstream /* WASI has no pipe */ ||           \
    defined(__wasip3__)
int pipe(int[2]);
int pipe2(int[2], int);
#endif
...
#define lseek(fd, offset, whence)                                              \
  ({                                                                           \
    ...
    off_t __o = (offset);                                                      \
    off_t __w = (whence);                                                      \
    __builtin_constant_p((offset)) && __builtin_constant_p((whence)) &&        \
            __o == 0 && __w == SEEK_CUR                                        \
        ? __wasilibc_tell(__f)                                                 \
        : lseek(__f, __o, __w);                                                \
  })
...
__attribute__((__deprecated__(
    "WASI lacks process identifiers; to enable emulation of the `getpid` "
    "function using "
    "a placeholder value, which doesn't reflect the host PID of the program, "
    "compile with -D_WASI_EMULATED_GETPID and link with "
    "-lwasi-emulated-getpid"))) pid_t getpid(void);
#endif
...
#define _POSIX_VDISABLE 0
    ...
    defined(__wasip3__) || !defined(_WASI_STRICT_PTHREAD)
#define _POSIX_THREADS _POSIX_VERSION
#define _POSIX_THREAD_PROCESS_SHARED _POSIX_VERSION
#define _POSIX_THREAD_SAFE_FUNCTIONS _POSIX_VERSION
#endif
#if defined(__wasilibc_unmodified_upstream) /* wasi-libc doesn't provide       \
                                               pthread_attr_{get,set}stackaddr \
                                             */
...
#endif
#if defined(__wasilibc_unmodified_upstream) /* WASI has no scheduling control, \
                                               and wasi-libc doesn't provide   \
...
#ifdef __wasilibc_unmodified_upstream /* WASI has no POSIX async I/O,          \
