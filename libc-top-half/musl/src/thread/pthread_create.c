...
int thread_new_indirect(void (*start)(void *), void *context);
void thread_yield_to(int);
#include <stdatomic.h>
#endif
...
struct start_args
{
	 ...
	 * in this structure is hardcoded in wasi_thread_start/wasm_coop_thread_start.
	 */
	void *stack;
	void *tls_base;
	void *(*start_func)(void *);
...
#ifdef __wasilibc_thread_model_posix
/*
...
hidden void __wasip3_thread_start_C(int tid, void *p)
{
	struct start_args *args = p;
	pthread_t self = __pthread_self();
	// Set the thread ID (TID) on the pthread structure. The TID is stored
...
#ifdef __wasilibc_use_wasip3
	/* TODO(sy)*/
	ret = thread_new_indirect(wasip3_thread_start, (void *)args);
#endif
#endif
